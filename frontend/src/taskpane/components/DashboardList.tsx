...
const DashboardList: React.FC = () => {
  ...
  return (
    <Layout style={{ padding: "24px", background: "#f0f2f5", minHeight: "100vh" }}>
              ...
              <List.Item>
                <Card
                  ...
                  actions={[
                    ...
                    <Tooltip title="Share Dashboard" key="share">
                      <Button type="text" icon={<UserAddOutlined />} onClick={() => openShareModal(dashboard.id)} />
                    </Tooltip>,
                    <Tooltip title="Edit Dashboard" key="edit">
                      <Button type="text" icon={<EditOutlined />} onClick={() => handleView(dashboard.id)} />
                    </Tooltip>,
                    <Tooltip title="Delete Dashboard" key="delete">
                      <Button type="text" danger icon={<DeleteOutlined />} onClick={() => handleDelete(dashboard.id)} />
                    </Tooltip>,
                  ]}
                  style={{
                    borderRadius: "12px",
                ...
                >
                  <Card.Meta
                    ...
                    description={<span style={{ color: "#595959" }}> Components: {dashboard.components.length} </span>}
                  />
                </Card>
              </List.Item>
            )}
      ...
      >
        <Input value={editTitle} onChange={(e) => setEditTitle(e.target.value)} placeholder="New dashboard title" />
      </Modal>
      <Modal title="Share Dashboard" open={isShareModalVisible} footer={null} onCancel={closeShareModal}>
        <Input
        ...
        {searchingUsers ? (
          ...
          <List
            ...
            renderItem={(user) => (
              <List.Item onClick={() => handleSelectUserToShare(user.userEmail)} style={{ cursor: "pointer" }}>
                <List.Item.Meta title={user.userEmail} description={user.fullName || user.userEmail} />
              </List.Item>
            )}
          />
        )}
